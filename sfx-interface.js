...
SfxInterface.prototype.setVolume = function(volume, filename) {
	this.Volume = volume;
	
	var filenames = this.audioObjects;
	if(filename)
	{
		filenames = {};
		filenames[filename] = this.audioObjects[filename];
	}
	
	for(var fname in filenames)
	{
		for(var i = 0; i < this.audioObjects[fname].length; i++)
		{
			this.audioObjects[fname][i].volume = this.Volume;
		}
	}
};

SfxInterface.prototype.setVolumeAt = function(index, volume) {
	this.setVolume(volume, this.fileNames[index]);
};
