var FW = {
	...
	'allImages': [],
	'SfxInterface': null,
	'RandomBlasting':false,
	'RandomBlastingLength':0,
	...
	StartRandomBlasts: function(length, level) {
		FW.RandomBlasting = true;
		FW.RandomBlastingLength = length;
		FW.RandomBlastingLevel = level;
		FW.RandomBlasts();
	},
	
	RandomBlasts: function() {
		
		if(FW.RandomBlastingLength > 0 && FW.RandomBlasting) {
			var amt = FW.Util.NextRandom(0, FW.RandomBlastingLevel + 1);
			for(var i = 0; i < amt; i++)
			{
				var x = FW.Util.NextRandom(0, FW.foregroundLayer.canvas.width);
				var y = FW.Util.NextRandom(0, FW.foregroundLayer.canvas.height);
				FW.CreateBlast(x, y);
			}
			
			FW.RandomBlastingLength--;
			setTimeout(FW.RandomBlasts, 300);
		}
	},
	
	StopRandomBlasts: function() {
		FW.RandomBlasting = false;
	},
